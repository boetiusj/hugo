...
1. **config.toml TOML scoping bug → junk `/true/` page + RSS never disabled.** `disableKinds = ['RSS']` and `enableRobotsTXT = true` sat *below* `[taxonomies]`, so TOML parsed them as taxonomy definitions. Hugo therefore created a taxonomy whose plural is literally "true" → **`crestwoodpainting.com/true/` was live, indexable, duplicate-listing junk**, and RSS files generated everywhere despite the config's intent. Fixed: `disableKinds` moved to top level; `enableRobotsTXT` dropped (static robots.txt serves, behavior unchanged). *(Batch-1 finding: the junk `/true/` taxonomy was also displacing the /ai/ section list to `/ai/ai/`; with the fix it renders at `/ai/` as intended. `/true/`, `/ai/ai/` and `/pages/` will 404 after deploy — all were unlinked junk; the indexable `/true/` will drop out of indexes naturally.)*
2. **Dead `hero_bg` frontmatter** (downgraded from bug during fix — the IsHome branch in head.html preloads `hero.image`, which is correct, so no 404 preload was actually emitted). The param pointed at nonexistent `/images/hero/home.webp`. Fixed: line removed.
3. **tulips2.webp CSS rule** (downgraded — the `.bg-1` block was already commented out, so no live request; the file referenced doesn't exist). Fixed: dead block deleted.
4. **Sidebar Categories widget broken on all 36 blog posts.** `blog/tag.html` built hrefs as `/categories/` + `urlize .Name`, but Hugo's modern `.Name` already includes the path → every post's sidebar emitted 6+ dead `/categories/categories/<term>/` links, raw `categories/<term>` labels, and a hardcoded `default 2` count. Fixed: link via `.Page.RelPermalink` / `.Page.LinkTitle` / real `.Count`. Bonus: schema-breadcrumb-ld.html now skips unrendered ancestors, so suppressing `/pages/` yields valid 2-level breadcrumbs instead of an empty `item`.
5. **Three main-nav items route through alias redirect stubs**: Email Us → `/contact/`, 12 Best Questions → `/12-questions-ask-painter/`, FAQs → `/faqs/`. Fixed: nav.yaml now links the canonical URLs (`/contact-us/`, `/12-top-painter-questions/`, `/frequently-asked-questions/`).
